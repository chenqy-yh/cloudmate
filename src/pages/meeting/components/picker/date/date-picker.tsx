...
const DatePicker: React.FC<DatePickerProps> = (props) => {
  const { value, title, important = false, placeholder = "请选择", onChange } = props

  const [inner_value, setInnerValue] = useState(value ?? getCurDay())
  const [isSelectorVisible, setSelectorVisible] = useState(false)

  const display_value = useMemo(() => (value ? formatDateDisplay(value) : placeholder), [placeholder, value])
    ...
    <>
      <div className={styles.date_picker} onClick={handleShowSelector}>
        ...
        <div className={styles.date_picker_bar}>
          ...
          <div className={value ? styles.choose_day : styles.placeholder}>{display_value}</div>
          {value && <Icon icon="close" size={20} className={styles.clear} onClick={handleClear} />}
