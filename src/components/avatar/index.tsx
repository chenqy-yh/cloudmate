...
const Avatar: React.FC<AvatarProps> = ({
  ...
  const renderAvatar = () =>
    src ? (
      <Icon
        className={classNames(styles.pic, className)}
        type="png"
        icon="user-1"
        size={size}
        {...rest}
  ...
  return (
    ...
    >
      ...
      {bottomLeftTag && (
        <div className={styles.bottom_left_tag}>{bottomLeftTag}</div>
      )}
      {bottomRightTag && (
        <div className={styles.bottom_right_tag}>{bottomRightTag}</div>
      )}
    </div>
  );
};
