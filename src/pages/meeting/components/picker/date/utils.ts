...
export const isValidDay = (day: Day): day is Day => {
  return (
    typeof day?.year === 'number' &&
    typeof day?.month === 'number' &&
    typeof day?.day === 'number' &&
    typeof day?.hour === 'number' &&
    typeof day?.minute === 'number' &&
...
export const dayTransfer = (day: Day) => {
  ...
  const { year, month, day: d, hour, minute } = day
  return new Date(year, month - 1, d, hour, minute);
}
