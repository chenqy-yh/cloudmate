import { punch } from "@/apis/attandence"
...
const AttendanceButton = () => {
  ...
  const handleCheckIn = async () => {
    ...
    setLoading(false)
    if (punch_res.statusCode === POST_SUCCESS) {
      ...
      const errMsg = (punch_res.data as unknown as ResponseError).message || punch_res.errMsg
      Taro.showToast({
        title: errMsg,
        icon: "error",
        duration: 2000,
  ...
  // 需要打卡
  return (
    <Button className={styles.atd_btn} onClick={handleCheckIn} disabled={loading || punch_record[cur_sign_in_type] !== null}>
      {loading ? <Loader version={1} /> : sign_in_types[cur_sign_in_type]}
    </Button>
  )
}
