...
const TabbarItem: React.FC<TabbarItemProps> = ({
  ...
  return (
    <div className={tabbarItemCls} onClick={() => handleChangeActiveTabbar()}>
      <Image
        className={styles.tabbar_item_icon}
...
const CustomTabbar = () => {
  ...
  const tabbarItemBackgroundCls = cls(
    styles.tabbar_item_background,
    styles[`index_${activeTabbarIndex}`]
  );
  ...
  const renderTabbarItems = () =>
    tabbar_item_config.map((config) => (
      <TabbarItem
        key={config.text}
        {...config}
        active={config.id === activeTabbarIndex}
        setActiveIndex={handleSetAactiveTabbar}
        navigate={navigate}
      />
    ));
